...
const BarItem = ({name,icon,link,active, eventHandler, index}) => {
    return (
        <div onClick={() => eventHandler(index)} className={`w-full p-5 hover:bg-purple-50 duration-300 ${index === active ? activeClass : ''}`}>
...
const SideBar = () => {
    ...
    return (
        <div>
            <div className='w-80 shadow-xl h-screen'>
                <div className='w-full flex flex-col'>
                    {
                        sideBarItems.map((obj, index) => {
                            return (
                                <BarItem eventHandler={handleClick} index={index} key={obj.name} name={obj.name} icon={obj.icon} link={obj.link} active={active}/>
                            )
                        })
                    }  
                </div>
